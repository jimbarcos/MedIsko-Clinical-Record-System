...
class ItemTemplate1_copy(ItemTemplate1_copyTemplate):
  ...
  def initDisplay(self):
    self.text_area_1.text = self.item["items"]
  ...
  def verifyInput(self):
    ...
    if (
      ...
      or self.txtQuantity.text is None
      or self.txtQuantity.text < 0
    ):
    ...
    if (
      self.txtServiceable.text == ""
      or self.txtServiceable.text is None
      or self.txtServiceable.text < 0
    ):
    ...
    if (
      self.txtForRepair.text == ""
      or self.txtForRepair.text is None
      or self.txtForRepair.text < 0
    ):
    ...
    if (
      self.txtForCondemn.text == ""
      or self.txtForCondemn.text is None
      or self.txtForCondemn.text < 0
    ):
    ...
    if (
      self.txtNeedReplacement.text == ""
      or self.txtNeedReplacement.text is None
      or self.txtNeedReplacement.text < 0
    ):
    ...
    if (
      ...
      or self.txtAdditional.text is None
      or self.txtAdditional.text < 0
    ):
    ...
    if (
      self.txtQuantityRequest.text == ""
      or self.txtQuantityRequest.text is None
      or self.txtQuantityRequest.text < 0
    ):
      Notification(
        "Invalid Component: Please Complete the Quantity of Request Details"
      ).show()
      error = True
      return error
    pass
    ...
    if not error:
      ...
      try:
        self.myInventory.add_row(
          ...
          quantityRequest=self.txtQuantityRequest.text,
        )
  ...
  def txtItems_pressed_enter(self, **event_args):
